...
Militarized Interstate Disputes (Dataset 2) This dataset records instances where states threaten, display, or use force against one another. The data includes details such as the start and end dates of conflicts, number of combatants, casualties, and the outcomes of these disputes.
...
library(tidyverse)
library(readxl)
library(countrycode) #this isn't working for me?
library(tidymodels)
library(openintro)
...
country_join_shorter <- country_join_edit |>
  select(stabb, styear, count, milex, milper, irst, cinc, hostlev) |>
  filter(styear>1950) |>
  distinct()
...
asia_25 <- asia |>
  filter(stabb %in% top_25_countries_in_asia)
...
#Doing EDA on the side

```{r}
#| label: explore
select_country_join <- country_join_edit |>
  select(stabb, styear, count, milex, milper, irst) |>
  filter(irst != 0) |>
  filter(styear > 1990) |>
  distinct()
select_country_join
ggplot(select_country_join, aes(x = count, y = irst)) +
  geom_point()
big <- select_country_join |>
  filter(stabb == "RUS" | stabb == "USA" | stabb == "GMY" | stabb == "CHN" | stabb == "IND" | stabb == "JPN")
ggplot(big, aes(x = styear, y = irst, color = stabb)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE)
ggplot(big, aes(x = styear, y = milex, color = stabb)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE)
ggplot(big, aes(x = styear, y = count, color = stabb)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE)
select_country_join |>
  summarize(mean_irst = mean(irst), median = median(irst))
select_country_join |>
  summarize(mean_milex = mean(milex), median = median(milex))
select_country_join |>
  summarize(mean_count = mean(count), median = median(count))
```
