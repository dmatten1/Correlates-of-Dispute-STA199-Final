...
CountryName <- data.frame(
  COWn = c("ZIM", "ZAN", "ZAM", "YUG", "YPR", "YEM", "YAR", "WSM", "WRT", "VEN",
           ...
           "TUR", "TUN", "TRI", "TON", "TOG", "TKM", "THI", "TAZ", "TAW", "TAJ", "SYR", "SWZ", "SWD", "SWA", "SVG", 
...
We were interested in what periods were their heightened levels of both conflict and military capabilities, focusing on Iron. @fig-period-conflicts shows that they both increase around the same time indicating that there could be a potential relationship and its a worthy quesiton to pursue. 

```{r}
#| label: fig-period-conflicts
#| fig-cap: "Trends in Conflict and Iron Production"
#| message: false
#| warning: false
#| layout-ncol: 2
...
This matrix provides a foundation understanding of how key material capabilities such as military expenditure, personnel, and iron production are related to conflict frequency. This graph establishes whether material capabilities are significantly correlated with conflict frequency. This is done in order to justify the variables selected for deeper analyses, showing which ones have potentially stronger predictive power. @fig-correlation-matrix shows that military expenditure and iron production has the highest correlation among the variables. 
...
observed_mean_milex <- asia_hypothesis |>
  specify(response = count, explanatory = milex_class) |>
  calculate(stat = "diff in means", order = c("high", "low")) |>
  pull(stat)
null_distribution_milex |>
  get_p_value(obs_stat = observed_mean_milex, direction = "two-sided")
null_distribution_milex |>
  get_confidence_interval()

null_distribution_irst <- asia_hypothesis |>
  specify(response = count, explanatory = irst_level) |>
  hypothesize(null = "independence") |>
  generate(reps = 1000, type = "permute") |>
  calculate(stat = "diff in means", order = c("High", "Low"))
observed_mean_irst <- asia_hypothesis |>
  specify(response = count, explanatory = irst_level) |>
  calculate(stat = "diff in means", order = c("High", "Low")) |>
  pull(stat)
null_distribution_irst |>
  get_p_value(obs_stat = observed_mean_irst, direction = "two-sided")
null_distribution_irst |>
  get_confidence_interval()
```
...
In order to determine outliers and potential spaces in our analysis 
a\.
iron
