...
Our hypothesis tests were based on the difference in medians rather than difference in means because of how heavily skewed our data was.

The null and alternative hypotheses for our hypothesis tests are shown below:
...
*Figure 1*: Reflects that out of all the continents, Asia has the highest number of total conflicts. 

*Figure 2/Figure 3*: Both scatter plots reveal that there is not a linear relationship between military expenditure (milex) or iron and steel production (irst) and total incident number.
In both visualizations, most of the total incident counts remain around 0, with milex mainly clustering between 0 to 5 \* 10\^7 thousands of international currency and iron and steel production mainly between 0 to 1.25 \* 10\^5 thousands of tons.
...
All aspects of our analysis point to an essentially-nonexistent linear correlation between the number of conflicts and any of the variables in our dataset.
...
Visualizations such as \<INSERT FIGURE NAMES HERE\> demonstrate this well; the linear regressions are positive, but they fit the data very poorly.
...
In general, with such a right skewed dataset with the median being equal to the low, models will not fit well at all.

The histogram \<INSERT FIGURE NAME\> comparing incident count and binary categories of \`milex_class\` and \`irst_class\` reflects this as well.
...
The same occurs for iron production in \<INSERT FIGURE NAME\>. 
