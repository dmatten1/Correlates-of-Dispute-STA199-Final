...
asia <- asia |> 
  mutate(
    milex_class = case_when(
      milex < quantile(milex, 0.50, na.rm = TRUE) ~ "low",
      .default = "high"
    ),
    
    irst_class = case_when(
      irst < quantile(irst, 0.50, na.rm = TRUE) ~ "low",
      .default = "high"
  )
)
```
...
ggplot(bootstrap_df, aes(x = boot_diff)) +
  geom_histogram(binwidth = 0.5, fill = "steelblue", color = "white") +
  labs(
    title = "Does High Military Personnel Correlate with Conflict Frequency?",
    x = "Military Personnel (milper)",
    y = "Average Number of Armed Conflicts"
  ) + 
  theme_minimal()
...
null_distribution_milex <- asia_hypothesis |>
  specify(response = count, explanatory = milex_level) |>
